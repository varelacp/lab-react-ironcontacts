...
Inside the `src` folder we have a `contacts.json` file containing the producer's contacts. Import the `contacts.json` file to `App.jsx`. Once done, create a state variable named `contacts` and store an **array containing the first 5 contacts**.

Display that array of 5 contacts as a list in a `<table>` and display the `picture`, `name`, and `popularity` of each contact.

For now, let's render the content in `App.jsx`. This being said, don't proceed to create a dedicated component for the contact list. The reason will become a bit clearer later when we add the delete button next to each contact. As a bonus you will be asked to refactor the components, lift state up and pass callbacks as props but for now we can continue with rendering everything in one component.

Let's proceed.

To import `contacts.json` in `App.jsx`, you can use:

```js
import contacts from './contacts.json';
```

There is already some starter code in `App.jsx` to help you format the `<table>` but remember that to display a list in React, you'll need to `.map()` over the elements and, in this case, return a `<tr>` for each contact. 😉
...
### Iteration 6 | Bonus | Refactor

We can refactor our code by extracting the parts of the existing HTML structure into a new component called `ContactRow`. We will need to pass as props all the details of the contacts but also "lift state up" and pass the delete callback as props to be able to have the _Delete_ button working again.

### Iteration 7 | Bonus | Styling
...
The warning _"Each child in a list should have a unique “key” prop"_ means that you are trying to render a list of elements, but one or more elements is missing the `key` prop.
